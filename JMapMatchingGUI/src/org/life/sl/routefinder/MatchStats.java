...
public class MatchStats {
	...
	public long nRejected_psOverlap = 0;
	...
	public void save(String fn, boolean writeHeader) {
		try {
			...
			if (writeHeader) {
				fw.write("# sourceID\tedges\tnodes\tmeanDegree\tmaxRoutes\ttrackLen\tODDist\tmaxLen\tmaxPSOv\tnLabels\tnRoutes\tnDeadEnds\tnRej_length\tnRej_overlap\trunTime\tstatus\n");
			}
			String s = String.format("%d\t%d\t%d\t%2.3f\t%2.3g\t", sourceRouteID, network_edges, network_nodes, network_meanDegree, network_maxRoutesEst); 
			s += String.format("%2.3f\t%2.3f\t%2.3f\t%2.4f\t%d\t%d\t%d\t%d\t%d\t%d\t", trackLength, ODDist, maxLength, maxPSOverlap, nLabels, nExpansions, nRoutes, nDeadEnds, nRejected_length, nRejected_overlap); 
